...
build_grey_herring <- function(herring, years) {
    ...
    compute_index <- function(dat, region, ages_5plus) {
        ...
        full_index <- full_index |>
            dplyr::mutate(
                age5plus_scaled = (.data$age5plus - scaling_mean) /
                    scaling_sd
            )
...
# In build_data.R, allow the aerial-count offset to be selected.
build_grey_aerial_counts <- function(
    aerial_counts,
    years,
    offset = 1L
) {
    aerial_counts <- aerial_counts |>
        reindex_years(
            year_col = "Year",
            years_keep = years,
            offset = offset
    ...
    list(
        n_aerial_years = nrow(aerial_counts),
        aerial_year = as.integer(aerial_counts$Year),
        obs_aerial_count = as.integer(aerial_counts$Count)
    )
}
