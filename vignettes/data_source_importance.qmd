...
```{r}
future_scenario_data <- build_scenario_data(
    rep(1000,times = 30), rep(1000, times = 30)
)
```
...
forecast_full <- forecast_ipm(
    fit_full,
    scenario_data = future_scenario_data,
    species = "grey",
    future_years = 2011:2040
)
...
p <- sealIPM:::forecast_plot(forecast_full, variables = c("birth_rate_future", "population_total_future"), future_years = 1:30) +
sealIPM:::forecast_plot(no_pregnancy_forecast, c("birth_rate_future", "population_total_future"), future_years = 1:30)
```
